//导出每种标签对应的品牌集合
//      { tag: tag_name, domain: domain_name, brands: brand1|brand2|...|brandN }
// 参考命令：
//      mongo 127.0.0.1:27017/ecomm export-b2c-tag-brands.js > b2c-tag-brands.json
    ...
    reduce: function (obj, prev) {
        ...
        function filter(brandArr, prev) {
            ...
            for (var i in brandArr) {
                ...
                if (prev.taboo.indexOf(myBrand) != -1 || prev.countries.indexOf(myBrand) != -1 || reNum.test(myBrand) || reG.test(myBrand)) {
                    continue;
                }
                if (myBrand.indexOf("其他") != -1) {
                    continue;
                }
                rst.push(myBrand);
            }
        ...
        function action(obj, prev) {
            var brand = obj.properties.品牌;
            ...
            brandArr = filter(brandArr, prev);

            for (var i in brandArr) {
                var myBrand = brandArr[i];
                if (prev.brandsDict[myBrand] == undefined) {
                    prev.brandsDict[myBrand] = 1;
                } else {
                    prev.brandsDict[myBrand] = prev.brandsDict[myBrand] + 1;
                }
            }
        }
    ...
    finalize: function (prev) {
        ...
        for (var myBrand in prev.brandsDict) {
            ...
            if (prev.brands.length == 0) {
                ...
                prev.brands = prev.brands + "|" + myBrand;
            }
        }
        delete prev.taboo;
        delete prev.countries;
        delete prev.brandsDict;
    }
});
printjson(result);
